/* common.es6.js
 	...
 	date, 2022/04/14	
*/
// This is an immediately invoked function expression (IIFE) that takes in the global window object as a parameter
!(function ($w) {
  "use strict";

  // If the window object is falsy, return
  if (!!!$w) return;

  // If the console object does not exist in the window object, create a new console object
  if (!!!$w["console"]) {
    // Create an empty array to store logs
    $w.logStack = [];

    // Create a new console object with log, dir, wtf, warn, and error methods that push messages to the logStack array
    $w.console = {
...
!(function () {
  ...
  String.prototype.removeRegExpChar = function (pattern) {
    return pattern == null
      ? this
      : eval("this.replace(/[" + pattern.meta() + ']/g, "")');
  };
  ...
  String.prototype.isUrl = function () {
    return /(file|ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/.test(
      this.trim()
    );
  };
  ...
  String.prototype.parseFloat = function () {
    if (arguments.length) {
      return String(this.trim().replace(/[^-_0-9.0-9]/g, ""))
        .Float()
        .toFixed(arguments[0])
        .parseFloat();
    } else {
  ...
  String.prototype.startWith = function (str) {
    ...
    if (str.length > 0) return str.equals(this.substring(0, str.length));
    else return false;
  };
  ...
  String.prototype.remove = function (str) {
    if (str && typeof str == "string") return this.replaceAll(str, "");
    else return this;
  };
  ...
  String.prototype.getFileName = function () {
    ...
    else return this.split("\\")[this.split("\\").length - 1];
  };
  ...
  String.prototype.isDate = function (spacter) {
    ...
    // yyyy-mm-dd
    if (
      /^(1|2)\d{3}[\/-](0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])$/.test(sz)
    )
      return true;
    // mm-dd-yyyy
    if (
      /^(0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])[\/-](1|2)\d{3}$/.test(sz)
    )
      return true;
    // yy-mm-dd
    if (/^\d{2}[\/-](0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])$/.test(sz))
      return true;
    // mm-dd-yy
    if (/^(0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])[\/-]\d{2}$/.test(sz))
      return true;
  ...
  String.prototype.isDateTime = function (spacter) {
    ...
    // yyyy-mm-dd hh:mm:ss
    if (
      /^(1|2)\d{3}[\/-](0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1]) (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
        sz
      )
    )
      return true;
    // mm-dd-yyyy hh:mm:ss
    if (
      /^(0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])[\/-](1|2)\d{3} (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
        sz
      )
    )
      return true;
    // yy-mm-dd hh:mm:ss
    if (
      /^\d{2}[\/-](0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1]) (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
        sz
      )
    )
      return true;
    // mm-dd-yyyy hh:mm:ss
    if (
      /^(0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])[\/-]\d{2} (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
        sz
      )
    )
      return true;
  ...
  String.prototype.toDate = function () {
    ...
    if (this.trim().toLowerCase() === "today")
      return new Date().format("yyyy-MM-dd").toDate();
    if (this.trim().toLowerCase() === "tomorrow")
      return new Date().addDay(1).format("yyyy-MM-dd").toDate();
    if (this.trim().toLowerCase() === "yesterday")
      return new Date().addDay(-1).format("yyyy-MM-dd").toDate();
    let arr = [];
    let min = 1900;
    let max = 3000;
    ...
    if (+str > 0 && "y,m,d,w,h".split(",").includes(fix)) {
      ...
      switch (fix) {
        case "y":
          ...
          break;
        case "m":
          rst = now.addMonth(val);
          break;
        case "d":
          rst = now.addDay(val);
          break;
        case "h":
          rst = now.addHours(val);
          break;
        case "w":
          rst = now.addDay(val * 7 - (val < 0 ? -1 : 1));
          break;
      }
      return rst;
      ...
      if (
        /^(19|29)\d{2}[\/\.-](0[1-9]|1[012])[\/\.-](0[1-9]|[12][0-9]|3[0-1])$/.test(
          srz
        )
      ) {
        ...
        /^\d{2}[\/\.-](0[1-9]|1[012])[\/\.-](0[1-9]|[12][0-9]|3[0-1])$/.test(
          srz
        )
      ) {
        ...
        /^(0[1-9]|1[012])[\/\.-](0[1-9]|[12][0-9]|3[0-1])[\/\.-](19|29)\d{2}$/.test(
          srz
        )
      ) {
        ...
        /^(0[1-9]|1[012])[\/-](0[1-9]|[12][0-9]|3[0-1])[\/-]\d{2}$/.test(srz)
      ) {
        // mm-dd-yy, mm/dd/yy, mm.dd.yy
        s = _split(srz);
        add(s[2] + min, s[0], s[1]);
      } else if (
        /^(19|29)\d{2}[\/\.-](0[1-9]|1[012])[\/\.-](0[1-9]|[12][0-9]|3[0-1]) (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
          srz
        )
      ) {
        ...
        t = srz
          .split(" ")[1]
          .split(":")
          .map((s) => String(s).parseInt());
        add(d[0], d[1], d[2], t[0] || 0, t[1] || 0, t[2] || 0);
      } else if (
        /^(0[1-9]|1[012])[\/\.-](0[1-9]|[12][0-9]|3[0-1])[\/\.-](19|29)\d{2} (2[0-3]|[0-1]\d)(:[0-5]\d){1,2}$/.test(
          srz
        )
      ) {
    ...
    return arr.length >= 3 && arr[0] >= min && arr[0] < max
      ? new Date(
          arr[0],
          arr[1] - 1,
          arr[2],
  ...
  Date.prototype.toJSON = function (key, format) {
    return String(
      isFinite(this.valueOf()) ? this.format(format || "F") : ""
    ).toJSON(key);
  };
  ...
  Date.prototype.getLastDay = function () {
    ...
    if (this.getMonth() == 1 && this.isLeapYear())
      // 윤년 2월달이면 일수가 다르다.
      return 29;
    else return String(days.split(",")[this.getMonth()]).Int();
  };
  ...
  Number.prototype.toMeridiem = function (type) {
    let meridiem = "";
    if (type == 1) meridiem = "AM,PM".split(",");
    else if (type == 2) meridiem = "오전,오후".split(",");
    else meridiem = "am,pm".split(",");
  ...
  Date.prototype.format = function (format, isLocal = true) {
      ...
      if (format && format.length == 1) {
        switch (format) {
          ...
          case "f": // "Wednesday, April  16, 2014 09:05 AM" LongDate+ShortTime
            return isLocal
              ? "yyyy-MM-dd, E2 Tt hh:mm"
              : "E3, MMMM dd, yyyy hh:mm TT";
          case "F": // "Wednesday, April  16, 2014 09:05:07 AM" FullDateTime
            return isLocal
              ? "yyyy년 MM월 dd일, E2 Tt hh시 mm분 ss초"
              : "E3, MMMM dd, yyyy hh:mm:ss TT";
    ...
    return formatting().replace(
      /(yyyy|yy|MMMM|MMM|MM|dddd|dd|yw|mw|E0|E1|E2|E3|HH|hh|mm|ssss|ss|tt|TT|Tt)/gi,
      function ($1) {
        switch ($1) {
          case "yyyy":
            return d.getFullYear();
          case "yy":
            return String(d.getFullYear() % 1000).digits(2);
          case "MMMM":
            return M[d.getMonth()];
          case "MMM":
            return m[d.getMonth()];
          case "MM":
            return String(d.getMonth() + 1).digits(2);
          case "dddd":
            return d.getDay().toWeekName(3);
          case "dd":
            return String(d.getDate()).digits(2);
          case "yw":
            return d.getWeek();
          case "mw":
            return d.getWeekOfMonth();
          case "E0":
            return d.getDay().toWeekName(0);
          case "E1":
            return d.getDay().toWeekName(1);
          case "E2":
            return d.getDay().toWeekName(2);
          case "E3":
            return d.getDay().toWeekName(3);
          case "HH":
            return String(d.getHours()).digits(2);
          case "hh":
            return String(d.getHours() % 12).digits(2);
          case "mm":
            return String(d.getMinutes()).digits(2);
          case "ss":
            return String(d.getSeconds()).digits(2);
          case "ssss":
            return d.getMilliseconds();
          case "tt":
            return d.getHours().toMeridiem(0);
          case "TT":
            return d.getHours().toMeridiem(1);
          case "Tt":
            return d.getHours().toMeridiem(2);
          default:
            return $1;
        }
      }
