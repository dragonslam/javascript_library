...
(function($w, root) {
    ...
    /** Public method. */
    Paser.parseToArray= function(cronstring) {
        return (cronstring||'').match(/^([\d,-]+|(?:\*|(?:\d{1,2}|\d{1,2}-\d{1,2}))\/\d{1,2}|\*{1})[ \n\t\b]+([\d,-]+|(?:\*|(?:\d{1,2}|\d{1,2}-\d{1,2}))\/\d{1,2}|\*{1})[ \n\t\b]+([\d,-]+|(?:\*|(?:\d{1,2}|\d{1,2}-\d{1,2}))\/\d{1,2}|\*{1})[ \n\t\b]+([\d,-]+|(?:\*|(?:\d{1,2}|\d{1,2}-\d{1,2}))\/\d{1,2}|\*{1})[ \n\t\b]+([\d,-]+|(?:\*|(?:\d{1,2}|\d{1,2}-\d{1,2}))\/\d{1,2}|\*{1})[ \n\t\b]*$/);
    };
    Paser.isCronPattern  = function(cronstring) {
        return (/^[\*]+$/.test(cronstring));
    };
    Paser.expressions = {
        YEARLY      : '@YEARLY',
        ANNUALLY    : '@ANNUALLY',
    ...
    * @api public
    */
    Paser.parse = function (cronstring, hasSeconds = false) {
        ...
        function parseExpr(expr) {
            ...
            for(field in FIELDS) {
                ...
                if (component && component !== '*' && component !== '?') {
                    ...
                    for (i = 0; i < length; i++) {
                        parse(items[i], schedule, field, f[1], f[2]);
                    }
                }
            }
